...
| Measured cost of the amendment | A metrics sweep is ~1.8 ms and a warm grouping pass ~2.9 ms on an M2 MacBook Air, so a full sample is roughly 6 ms — about 0.6% of one core at 1 Hz, against the ≤1% idle reference figure. Estimated at the time of the amendment and **to be replaced with a measured figure** from `probe/overhead/run.sh 300` against the running app, since the headless harness has historically under-read it badly. The amendment is to be reconsidered if the measured figure lands materially above the reference. |
