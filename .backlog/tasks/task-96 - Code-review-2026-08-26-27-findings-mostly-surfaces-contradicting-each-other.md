...
**Status: 25 of 27 fixed** across commits 6e7cd3a, 79a30f1, 2b13224 and 51b0f31. Only 15 and 27 remain, and both are deliberate — see the notes.
...
6. ✅ "No slowdowns since 9:14 AM" counted up to 90 days of persisted incidents against the launch time.
7. ✅ The popover named the live CPU leader while the Now banner named the recorded one — the exact thing `NowPresentation.bannerHeadline`'s comment forbids.
8. ✅ During a repeated-quit incident the popover's action offered to bring the CPU leader forward, not the subject of the headline above it.
9. ✅ "Relaunches while watching" was counted by 16-byte command across the whole machine and shown uncaveated and unlabelled on one application's inspector.
10. ✅ "Bring to front" was offered for daemons that cannot be activated — TASK-94's defect at a second site, plus TASK-94's own AC #3, both closed.
11. ✅ A Now footnote said applications have no retained history, under a table drawing their curves.

**Medium** — 12 ✅ memory budget stated in resident size (removed from the product entirely at the product owner's instruction) · 13 ✅ the Apps table sorted by a column that was not on screen · 14 ✅ the Now table's header detached from its columns exactly when readings went stale · **15 ⬜ the contributor row may not fit the window's minimum width — needs the screen** · 16 ✅ mounting a disk reported a fabricated write spike · 17 ✅ one sample of jitter deleted a family's whole retained series · 18 ✅ the verdict and the menu bar ignored the user's own thresholds · 19 ✅ the summariser's live fallback contradicted its own contract · 20 ✅ "none of your open apps accounts for it" said when up to 49% of it is your apps · 21 ✅ two safe actions reported success without evidence · 22 ✅ the Now screen rebuilt the whole inventory once a second.

**Low** — 23 ✅ Alerts tab described a defect TASK-69 fixed · 24 ✅ the menu bar sparkline had no baseline and amplified calm · 25 ✅ `StorageScreenModel.tick` had no caller · 26 ✅ the self-cost line did not name its statistic and showed a measured 0.0% before measuring · **27 ⬜ the Now contributor list does not visibly sum — needs a product decision**.
...
The two outstanding findings are tracked as tasks of their own so they are actionable rather than buried in this umbrella.
