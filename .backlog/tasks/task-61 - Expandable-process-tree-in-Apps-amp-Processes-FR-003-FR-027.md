...
Today the inventory shows one row per family with an aggregate figure, and the individual processes beneath it — which FR-003 requires we preserve — are not reachable from the interface at all. BrowserApp has 18 processes and AssistantApp 25; a user can see the total but not what makes it up.
...
Depends on TASK-60 for the cases that matter most. Grouping by executable path alone leaves 25 unbundled processes as their own top-level rows — 14 zsh under TerminalApp, 11 backlog under AssistantApp. Those are exactly the children a user would expect to find nested. Without ppid grouping, a TerminalApp session still appears as fourteen unrelated rows.
